        ...
        <div class="config_wrap">
            ...
            <div style="display: flex;align-items: center;margin-bottom: 20px">
                <el-checkbox @change="from.max_like_num = ''" style="margin-right: 53px" v-model="from.check_like"
                    label="点赞/条视频大于" />
                <el-input @blur="handleLikeBlur(val)" :disabled="!from.check_like" placeholder="请输入配置"
                    v-model="from.max_like_num" style="width: 120px;"></el-input>
            </div>

            <div style="display: flex;align-items: center;margin-bottom: 20px">
                <el-checkbox @change="from.max_comment_num = ''" style="margin-right: 53px" v-model="from.check_comment"
                    label="评论/条视频大于" />
                <el-input @blur="handleBlur(val)" :disabled="!from.check_comment" placeholder="请输入配置"
                    v-model="from.max_comment_num" style="width: 120px;"></el-input>
            </div>
            ...
            <div style="display: flex;justify-content: flex-end;">
                <el-button :disabled="props.categoryId === '' || props.categoryId === undefined"
                    @click="setLowExplosiveConfig" :loading="loading">保存</el-button>
            </div>
        </div>
    </div>
</template>
...
const handleBlur = (val) => {
    if (from.max_comment_num === '') {
        from.check_comment = false
    }
}

const handleLikeBlur = (val) => {
    if (from.max_like_num === '') {
        from.check_like = false
    }
}

const fetchLowExplosiveConfig = async (data = { class_id: props.categoryId }) => {
    try {
        loading.value = true
        const res = await $api({ type: 'fetchLowExplosiveConfig', data })
        if (res.code === 0) {
...
const setLowExplosiveConfig = async () => {
    try {
        loading.value = true
        const data = {
        ...
        const res = await $api({ type: 'setLowExplosiveConfig', data: postData })
        if (res.code === 0) [
